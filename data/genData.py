...
"""
@Author:sunlf
@Date:11:34 2018/9/12
@Description:
"""
# make black-white background images
def gen_bw_images(fontsize,label):
...
# make idcard background images
def gen_idCardBG_images(idcard_bg_path):
    j = random.randint(1,20)
    if j<10:
        j = "0"+str(j)
    img = Image.open(idcard_bg_path+"bg_"+str(j)+".png")
    draw = ImageDraw.Draw(img)
    hw = (random.randint(10,50),5)
    return img,draw,hw

# make randomly bw_images and idcard_images
def genData(i,label,fontsize,dataType,dataFormat,fontName):
    idcard_base_path = cfg.IDCARD_CFG.BG_BASE_PATH
    k = random.randint(1,22)
    output_path = cfg.OUTPUT_PATH
    if k<15:
        img,draw,hw = gen_idCardBG_images(idcard_base_path)
    else:
        img,draw,hw = gen_bw_images(fontsize,label)
    font = ImageFont.truetype(fontName, fontsize)
    draw.text(hw, label, fill=(0,0,0), font=font)
    img_path = output_path+"data_"+dataType+'_'+ str(i) +"_"+str(fontsize)+ dataFormat
    with open(output_path+"sample.txt",'a',encoding='utf-8') as file:
        file.write(img_path+'\t'+str(label)+'\n')
    img.save(img_path)


#gem text from national address
def genImageFromAddress(workbook,fontName,dataNum):
    sheet2 = workbook.sheet_by_index(0)
    city = ''
    center = ''
    region = ''
    street = ''
    for i in range(dataNum): #sheet2.nrows:all of index
        if(str(sheet2.cell(i,3))=="text:'1'"):
            city = str(sheet2.cell(i,2)).strip('text:').strip("'")
        if(str(sheet2.cell(i,3))=="text:'2'"):
            center = str(sheet2.cell(i,2)).strip('text:').strip("'")
        if(str(sheet2.cell(i,3))=="text:'3'" and str(sheet2.cell(i,4))=="text:'0'"):
            region = str(sheet2.cell(i,2)).strip('text:').strip("'")
        if(str(sheet2.cell(i,3))=="text:'4'" and str(sheet2.cell(i,4))=="text:'1'"):
            street = str(sheet2.cell(i,2)).strip('text:').strip("'")
            genData(i,city+center+region+street,15,'Address','.png',fontName)

def genImageFromAlphabet(fontName,dataNum):
    ...
    for i in range(dataNum):
        ...
        genData(i,alphabet[ss:ss + 10],15,'wb','.png',fontName)

if __name__ == '__main__':
    workbook = xlrd.open_workbook(r'national.xls')
    fontName = cfg.FONTTYPE
    dataNum = 10
    genImageFromAddress(workbook,fontName,dataNum)
    genImageFromAlphabet(fontName,dataNum)
